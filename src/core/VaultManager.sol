...
contract VaultManager is IVaultManager {
    ...
    function deposit(uint256 id, address vault, uint256 amount) public virtual {
        Vault _vault = Vault(vault);
        _vault.asset().safeTransferFrom(msg.sender, address(vault), amount);
        _vault.deposit(id, amount);
    }

    function withdraw(uint256 id, address vault, uint256 amount, address to) public virtual {
